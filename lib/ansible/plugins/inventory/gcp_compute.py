        ...
        for key in item:
            try:
                self.inventory.set_variable(
                    hostname, self.get_option("vars_prefix") + key, item[key]
                )
    ...
    def _add_hosts(self, items, config_data, format_items=True, project_disks=None):
        ...
        for host in items:
            ...
            self._set_composite_vars(
                self.get_option("compose"), host.__dict__(), hostname
            )
            self._add_host_to_composed_groups(
                self.get_option("groups"), host.__dict__(), hostname
            )
            self._add_host_to_keyed_groups(
                self.get_option("keyed_groups"), host.__dict__(), hostname
            )
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        for library in GcpSession.missing_libraries():
            raise AnsibleParserError(
                "gce inventory plugin cannot start: %s" % missing_required_lib(library)
            )
                ...
                for project in results:
                    for zone in results[project]:
                        self._add_hosts(
                            ...
                            config_data,
                            False,
                            project_disks=self.project_disks,
                        )
            except KeyError:
